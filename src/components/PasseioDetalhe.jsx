...
function calcPrecoPrivativo() {
  // Trekking: preço variável por pessoa conforme tabela
  if (passeio.id === 'trekking-lencois') {
    const chave = Math.min(pessoas, 6);
    return passeio.privativo?.[chave] || 0;
  }
  // Quadriciclo: R$ 650 por dupla (arredonda para cima)
  if (passeio.id === 'quadriciclo') {
    return Math.ceil(pessoas / 2) * 650;
  }
  // Demais privativos: valor fixo sempre
  return passeio.privativo?.[1] || passeio.privatePricePerPerson || 0;
}
  ...
  {passeio.id === 'trekking-lencois'
    ? `R$ ${(calcPrecoPrivativo() / pessoas).toLocaleString('pt-BR')}/pessoa`
    : passeio.id === 'quadriciclo'
    ? `${Math.ceil(pessoas / 2)} quadriciclo${Math.ceil(pessoas / 2) > 1 ? 's' : ''} × R$ 650`
    : 'valor fixo para o grupo'}
</p>
